...
# Proses produksi — urutan sesuai alur rajut PT Rahaza (UPDATED: QC after Steam)
DEFAULT_PROCESSES = [
    ...
    {"code": "STEAM",   "name": "Steam",   "order_seq": 4, "is_rework": False, "description": "Penyetrikaan uap"},
    {"code": "QC",      "name": "QC",      "order_seq": 5, "is_rework": False, "description": "Quality control"},
    {"code": "PACKING", "name": "Packing", "order_seq": 6, "is_rework": False, "description": "Pengemasan akhir"},
    {"code": "REWORK",  "name": "Rework",  "order_seq": 10,"is_rework": True,  "description": "Perbaikan untuk item yang gagal QC"},
]
...
async def seed_rahaza_master_data():
    ...
    # Deactivate old rework processes (WASHER, SONTEK) if they exist
    old_rework = ["WASHER", "SONTEK"]
    for code in old_rework:
        result = await db.rahaza_processes.update_one(
            {"code": code, "active": {"$ne": False}},
            {"$set": {"active": False, "updated_at": _now()}}
        )
        if result.modified_count > 0:
            print(f"  · Deactivated old rework process: {code}")
